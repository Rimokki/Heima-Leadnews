    ...
    const htmlStr = result.data.results
      .map(
        ...
        <td>
          ${
            item.status === '1'
              ? '<span class="badge text-bg-success">待审核</span>'
...
async function setChannelList() {
  const channelList = await axios({ url: '/v1_0/channels' })
  const htmlStr =
    '<option value="" selected>请选择文章频道</option>' +
    channelList.data.channels.map((item) => `<option value="${item.id}">${item.name}</option>`).join('')
...
document.querySelector('.art-list').addEventListener('click', async (e) => {
  if (e.target.classList.contains('del')) {
    ...
    const chilren = document.querySelector('.art-list').childNodes
    if (chilren.length === 1 && queryObj.page !== 1) {
      queryObj.page--
      document.querySelector('.page-now').innerHTML = `第 ${queryObj.page} 页`
    }
    setArticleList()
